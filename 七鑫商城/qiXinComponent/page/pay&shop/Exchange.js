...
export  default class Pay extends  Component{
    ...
    _pay(){
        ...
        if(!this.state.id){
            ...
            console.log('http://' + ipdy + '/index.php?s=/Home/APPGoods/buyGoods/uid/'+this.props.uid+'/paytype/1/address_id/'+this.state.id+'/detail/'+this.props.pid+'_'+this.props.num+'/price/'+this.props.price1*this.props.num+'/sign/'+(Cy('address_id='+this.state.id+'&detail='+this.props.pid+'_'+this.props.num+'&paytype=1&price='+this.props.price1*this.props.num+'&uid='+this.props.uid+'&key='+qm)))
        ...
        return(
            <View style={{flex:1,backgroundColor:'#ffffff'}}>
                ...
                <Modal animationType={'none'} style={{width:WINWIDTH,height:WINHEIGHT*(0.25),backgroundColor:'#f5f5f5',justifyContent:'center',alignItems:'center',borderRadius:10}} position={"bottom"} ref={"pay"}>
                    ...
                    <TouchableOpacity onPress={this._pay.bind(this)} style={{width:WINWIDTH*0.8,height:WINHEIGHT*0.06,backgroundColor:'#ffffff',justifyContent:'center',flexDirection:'row',alignItems:'center',borderRadius:5,shadowColor:'#515151',shadowOffset:{width:1,height:1},shadowRadius:1,shadowOpacity: 1.0,marginTop:10}}>
                        <View style={{justifyContent:'center',alignItems:'center',width:40,height:30}}>
                        <Image source={require('../../../img/hongbao.png')} style={{width:30,height:30}} />
                        </View>
                        <Text style={{fontSize:WINWIDTH*0.04,color:'#696969',marginLeft:10,fontWeight:'bold'}}>宝石支付</Text>
                    </TouchableOpacity>
